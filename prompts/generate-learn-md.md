...
출력을 작성한 뒤, 본문을 내보내기 직전에 다음을 검증하라. 어긋나면 수정 후 출력한다.

1. frontmatter의 `sentence_count` 값과 실제 `## S\d+` 헤딩 개수가 정확히 일치하는가
2. 모든 문장이 `### 번역`, `### 청크`, `### 구조`, `### 어휘`, `### 감각` 5개 서브섹션을 빠짐없이 가지는가
3. 각 문장의 첫 줄이 `> ` (인용)으로 시작하며 원문 그대로인가
4. 어휘 개수가 문장당 2~5개 범위인가
